...
@MainActor
final class CPUpdateChecker {

    static let shared = CPUpdateChecker()

    // Your repo
    private let owner = "EllandeVED"
    private let repo  = "Clip-Paste"  // TODO: change to your actual Clip Paste repo name if different
    ...
    private func showUpdateAlert(remoteTag: String, page: URL, directDownloadURL: URL?, notes: String?) {
        let alert = NSAlert()
        alert.window.title = "Clip Paste Update"
        alert.messageText = "A New Version of Clip Paste is Available"
        ...
        if alert.runModal() == .alertFirstButtonReturn {
            ...
            if let direct = directDownloadURL {
                ...
                let suggestedName = "Clip-Paste-\(cleanTag).zip"
                downloadAsset(from: direct, suggestedName: suggestedName)
            } else {
    ...
    private func downloadAsset(from url: URL, suggestedName: String = "Clip-Paste.zip") {
